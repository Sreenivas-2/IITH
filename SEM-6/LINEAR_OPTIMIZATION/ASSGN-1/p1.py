...
def initExtreme(A, b):
	'''
		...
		returns => Inital extreme point and the active set (indices of n linearly independent rows of A)
	'''
	...
	add = np.zeros((1, A_.shape[1]))
	add[0][-1] = 1

	A_ = np.vstack((A_, add))
	b_ = np.vstack((b, np.array([0]).reshape(-1,1)))

	indices.append(np.argmin(b))
	J = np.array(indices)
...
def get_DirVec(A, k, J):
	'''
		A => m*n matrix where each row consists of coefficients of the constraints
		k => index of element with z < 0 (row which shall be removed from active set)
		J => active set at current extreme point
